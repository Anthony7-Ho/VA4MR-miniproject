...
def setup_dataset(ds, paths):
    ...
    if ds == 0:  # KITTI
        ...
        data_params['ground_truth'] = np.loadtxt(os.path.join(kitti_path, "poses/05.txt"))[:, [-9, -8]]
        data_params['last_frame'] = 2760 #TODO: Ask Jiaxiu why 4540
        data_params['K'] = np.array([[718.856, 0, 607.1928],
                                     [0, 718.856, 185.2157],
                                     [0, 0, 1]])
    elif ds == 1:  # Malaga
        ...
        data_params['images'] = sorted([f for f in os.listdir(images_dir) if f.endswith('.jpg')])[2::2]
        data_params['path'] = images_dir
        data_params['last_frame'] = len(data_params['images']) - 1
        # print(data_params['last_frame'])
        data_params['K'] = np.array([[621.18428, 0, 404.0076],
        ...
        data_params['ground_truth'] = np.loadtxt(os.path.join(parking_path, "poses.txt"))[:, [-9, -8]]
    else:
...
def load_image(data_params, frame_idx, grayscale=False):
    ...
    if 'images' in data_params:
        ...
        if 'kitti' in data_params['path']:
            image_path = os.path.join(data_params['path'], f"05/image_0/{frame_idx:06d}.png")
        elif 'parking' in data_params['path']:
            image_path = os.path.join(data_params['path'], f"images/img_{frame_idx:05d}.png")
        else:
